...
module {
  ...
  public func init( fs : Filesystem, su : Principal, g : ?[Principal], m : ?Mode ) : () {
    ...
    fs.inodes := Prim.Array_init<Inode>(5000, #Directory({
      name = Text.Base.fromChar(Path.Slash);
      inode = 0;
      parent = 0;
      owner = su;
  ...
  public func walk( fs : Filesystem, p : Path, eid : Principal ) : Return<Inode> {
    ...
    switch( open(fs, p, eid, false, false) ){
      ...
      case ( #ok(index) ) {
        ...
        if ( is_permitted(dacl, eid, #read) or Principal.Base.equal(fs.root, eid) ) #ok(fs.inodes[index])
  ...
  public func save( fs : Filesystem, index : Index, eid : Principal, file : File ) : Return<()> {
    switch( fs.inodes[index] ){
      ...
      case ( #Reserved(oid) ){
        if ( not Principal.Base.equal(oid, eid) ) return #err(#Unauthorized);
  ...
  public func mkdir( fs : Filesystem, p: Path, eid : Principal, g : ?[Principal], m : ?Mode ) : Return<()> {
    switch( touch(fs, p, eid) ){
      ...
      case ( #ok(index) ){
        switch( fs.inodes[index] ){
          case ( #Reserved(oid) ){
            if ( Principal.Base.equal(oid, eid) == false ) return #err(#Unauthorized);
  ...
  public func make_root_folder(fs : Filesystem, h : Handle, eid : Principal, g : ?[Principal], m : ?Mode ) : Return<()> {
    let path = Path.join(Text.Base.fromChar(Path.Slash), h);
    switch( touch(fs, path, fs.root) ){
      case ( #err(val) ) #err val;
      case ( #ok(index) ){
        switch( fs.inodes[index] ){
          case ( #Reserved(oid) ){
            if ( Principal.Base.equal(oid, fs.root) == false ) return #err(#Unauthorized);
            fs.inodes[index] := #Directory({
              parent = 0;
              inode = index;
              name = h;
  ...
  func open( fs : Filesystem, p : Path, eid : Principal, _c : Bool, _h : Bool ) : Return<Index> {
    if ( not Path.is_absolute(p) ) return #err(#Invalid p);
    var inode = fs.inodes[0];
    if ( Path.is_root(p) ) return #ok(0);
    let target : Depth = Path.depth(p)-1;
    var current : Depth = 0;
    while( current <= target ){
      switch inode {
        case ( #Directory(dir) ){
          if ( is_permitted(dir, eid, #read) or Principal.Base.equal(fs.root, eid) ) {
            let elem = Option.get<Text>( Path.index(p, current), Text.Base.fromChar(Path.Slash) );
            switch ( Array.find<Dentry>(dir.contents, func(x) = x.2 == elem ) ) {
              case ( ?dentry ) {
                switch( dentry.3 ){
                  case ( #Valid ) {
                    if ( current == target ) {
                      if ( _c == false ) return #ok(dentry.0);
                      if ( is_permitted(dir,eid, #write) or fs.root == eid ){
  ...
  public func is_permitted(acl : DACL, eid : Principal, op : Operation) : Bool {
    ...
    if ( eid == acl.owner ) return true;
    var perms : Priv = #NO;
    switch ( Array.find<Principal>(acl.group, func x = x == eid ) ){
